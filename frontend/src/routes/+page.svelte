<script lang="ts">
  ...
  function describeSide(player1Id: string, player2Id?: string | null): string {
    ...
    const player2 = player2Id
      ? allPlayers.find((player) => player.id === player2Id)
      : undefined;
  ...
  async function handleDeletePlayer(
    ...
    const shouldDelete = confirm(`Delete player "${playerName}"?`);
    if (!shouldDelete) {
      return;
    }
...
<main class="page">
  <header class="hero">
    <div>
      ...
      <p class="subtitle">
        Log players and matches, then track standings from real results.
      </p>
    </div>
  </header>
  ...
  <section class="cards two-up">
    <article class="card">
      <h2>Add player</h2>
      <p class="card-help">
        Create someone once, then use them in every match.
      </p>
  ...
  <section class="card">
    <h2>Log match</h2>
    <p class="card-help">
      Supports both 1v1 and 2v2. Leave player 2 empty for 1v1.
    </p>
            ...
            <li class="list-item">
              <div>
                <p class="match-teams">
                  ...
                  <span class="score"
                    >{match.leftScore} - {match.rightScore}</span
                  >
